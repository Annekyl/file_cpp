...
char g[N][N]; // 存储游戏地图
Node pre[N][N]
        [4]; // 表示箱子在(x, y)，上一个格子在i方向上的状态，即上一个状态是(x +
             // dx[i], y + dy[i])
vector<int> path
    [N][N]
    [4]; // path[j][k][i]
         // 表示人从推(j,k,i)的上一个状态的位置，走到推(j,k,i)这个状态的位置的行走路径
bool st[N][N][4], used[N][N]; // BFS的判重数组，为了防止BFS遍历相同状态
PII dist
    [N][N]
    [4]; // dist[j][k][i]是表示从初始状态到达j,k,i状态所需要的箱子最短路程和人行走最短路程
int dx[4] = {1, -1, 0, 0},
    dy[4] = {0, 0, 1, -1}; // 依次表示下、上、右、左四个方向
int pre_man[N][N]; // pre_man[x][y]表示人从哪个方向走到(x, y)，如果pre_man[x][y]
                   // = i, 那么上一个状态是(x - dx[i], y - dy[i])
    ...
    while (q.size()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (check(a, b) && check(j, k)) {
                vector<int> seq;
                auto &p = dist[j][k][i];
                int distance = bfs_man({t.x + dx[t.dir], t.y + dy[t.dir]},
                                       {a, b}, {t.x, t.y}, seq);
                if (distance != -1) {
                    PII td = {dist[t.x][t.y][t.dir].first + 1,
                              dist[t.x][t.y][t.dir].second + distance};
                    if (!st[j][k][i]) {
                        st[j][k][i] = true;
                        q.push({j, k, i});
                        path[j][k][i] = seq;
